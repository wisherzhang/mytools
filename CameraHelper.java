...
//                YUV420co(yuv_Rotate90,mheight,mwidth,yuv_frame,mheight, mwidth);
//                YUV420sc(yuv_frame,mheight, mwidth,yuv_Rotate90,mheight,mwidth);
//                callHelper.processPreviewData(mheight, mwidth, yuv_Rotate90);
                
                yuv_frame = Arrays.copyOf(yuv_Rotate90, yuv_Rotate90.length);
                byte [] temp_yuv = new byte[120*160*3/2];
                YUV420co(yuv_Rotate90,mheight,mwidth,temp_yuv,120, 160);
                YUV420sc(temp_yuv,120, 160,yuv_frame,mheight,mwidth);
                callHelper.processPreviewData(mheight, mwidth, yuv_frame);
            } else {
            	Log.i(TAG,"cameraInfo.orientation :" + cameraInfo.orientation +" mheight :"+mheight+" mwidth :" + mwidth);
                if (cameraInfo.orientation == 90 || cameraInfo.orientation == 0 || cameraInfo.orientation == 180) {
    ...
    void YUV420sc(byte[] src, int srcWidth, int srcHeight, byte[] dst, int dstWidth, int dstHeight)
    {
    	int w_cut = (dstWidth -srcWidth+1)/2;
        int h_cut = (dstHeight- srcHeight +1)/2;
        for (int row = 0; row < dstHeight-2*h_cut; ++row) {
            for (int col = 0; col < dstWidth-2*w_cut; ++col) {
                
            	dst[row*dstWidth+col+w_cut+h_cut*dstWidth] = src[row*srcWidth+col];
            }
        }
        
        int k = 0;
        int ylength = dstWidth*dstHeight;
        int ulength = dstWidth*dstHeight/4;        
        for (int row = 0; row < dstHeight/2-h_cut*2; row+=1) {
            for (int col = 0; col < dstWidth/2-w_cut*2; col+=1) {
                int old_index = (row)*srcWidth/2+col;
                dst[row*dstWidth/2+col+w_cut+ylength+h_cut*dstWidth/4] = src[old_index+ylength];
                dst[row*dstWidth/2+col+w_cut+ylength+ulength+h_cut*dstWidth/4] = src[old_index+ylength+ulength];
                k++;
            }
        }   
    }
    void YUV420co(byte[] src, int srcWidth, int srcHeight, byte[] dst, int dstWidth, int dstHeight)
    {
    	int w_cut = srcWidth - dstWidth;
        int h_cut = srcHeight- dstHeight;
        
        for (int row = 0; row < dstHeight; ++row) {
            for (int col = 0; col < dstWidth; ++col) {
                
            	dst[row*dstWidth+col] = src[(h_cut+row)*srcWidth+(w_cut+col)];
            }
        }
        ...
        int ylength = dstWidth*dstHeight;
        int ulength = dstWidth*dstHeight/4;        
        for (int row = 0; row < dstHeight; row+=2) {
            for (int col = 0; col < dstWidth; col+=2) {
                int old_index = (h_cut+row)*srcWidth/4+(w_cut+col)/2;
                dst[k+ylength] = src[old_index+ylength];
                dst[k+ylength+ulength] = src[old_index+ylength+ulength];
                k++;
            }
        }        
    }
